...
CREATE UNIQUE INDEX IF NOT EXISTS iu_cliente_cd ON public.cliente_old USING btree (xcd_cliente) WHERE (excluido_visivel = false);
CREATE UNIQUE INDEX IF NOT EXISTS iu_deposito_empresa_nome ON public.deposito USING btree (empresa_id, nome) WHERE (excluido_visivel = false);
...
CREATE UNIQUE INDEX IF NOT EXISTS iu_grupo_cadastro_empresa_nome ON public.grupo_cadastro USING btree (empresa_id, nome) WHERE (excluido_visivel = false);
CREATE UNIQUE INDEX IF NOT EXISTS iu_grupo_produto_cd ON public.grupo_produto_old USING btree (xcd_grupo_produto) WHERE (excluido_visivel = false);
CREATE UNIQUE INDEX IF NOT EXISTS iu_movimento_empresa ON public.movimento USING btree (empresa_id, movimento_id);
CREATE UNIQUE INDEX IF NOT EXISTS iu_produto_barra ON public.produto_old USING btree (xcd_barra) WHERE (xcd_barra IS NOT NULL AND xcd_barra <> '' AND excluido_visivel = false);
CREATE UNIQUE INDEX IF NOT EXISTS iu_produto_cd ON public.produto_old USING btree (xcd_produto) WHERE (excluido_visivel = false);
...
CREATE OR REPLACE FUNCTION public.fu_get_cliente_public(_cpf text) RETURNS TABLE(id bigint, razao_social text, fone_geral text, dep_nome1 text)
  ...
  FROM public.cadastro c WHERE c.excluido_visivel = false
...
CREATE OR REPLACE FUNCTION public.fu_get_pedido_status_public(_movimento_id bigint, _cpf text) RETURNS TABLE(id bigint, nr_movimento bigint, st_pedido text, dt_emissao timestamp with time zone, vl_movimento numeric, url_pagamento text)
  ...
  WHERE m.movimento_id = _movimento_id AND m.excluido_visivel = false AND m.tp_origem = 'LINK'
...
CREATE OR REPLACE FUNCTION public.fu_list_pedidos_public(_cpf text) RETURNS TABLE(id bigint, nr_movimento bigint, dt_emissao timestamp with time zone, vl_movimento numeric, st_pedido text, items jsonb)
  ...
  FROM public.movimento m JOIN public.cadastro c ON c.cadastro_id = m.cadastro_id
  LEFT JOIN public.movimento_item mi ON mi.movimento_id = m.movimento_id AND mi.excluido_visivel = false
  WHERE m.excluido_visivel = false AND m.tp_origem = 'LINK' AND c.cnpj = regexp_replace(coalesce(_cpf, ''), '\D', '', 'g')
...
BEGIN
  ...
  SELECT c.cadastro_id INTO v_id FROM public.cadastro c WHERE c.excluido_visivel = false AND c.cnpj = v_cpf ORDER BY c.cadastro_id DESC LIMIT 1;
...
CREATE OR REPLACE VIEW public.vw_pedidos_completos AS
 SELECT m.movimento_id, m.empresa_id, m.tp_movimento, m.nr_movimento, m.cadastro_id, m.dt_emissao, m.hr_movimento,
    ...
    m.obs_pedido, m.id_transacao_abacatepay, m.url_pagamento, m.qr_code_pagamento, m.excluido_visivel,
...
CREATE OR REPLACE VIEW public.vw_produtos_disponiveis AS
 SELECT p.produto_id, p.empresa_id, p.grupo_id, p.nome, p.nome_reduzido, p.descricao, p.unidade_id, p.gtin,
    ...
    p.preco_sugerido, p.url_foto, p.venda_online, p.dias_venda_online, p.controla_estoque, p.excluido_visivel,
