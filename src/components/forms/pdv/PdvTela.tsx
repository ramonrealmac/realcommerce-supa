...
import { LogOut, Search, Trash2, Plus, Receipt, RefreshCw } from "lucide-react";
...
import type { IPdvCaixa, IPdvCaixaAbertura, IPdvParamsEmpresa, IPdvPedidoFechado, IPdvPagamentoLinha } from "./types";
...
const PdvTela: React.FC<IProps> = ({ caixa, abertura, dtMovimento, onSair }) => {
  ...
  useEffect(() => {
    (async () => {
      const { data, error } = await db.from("empresa")
        .select("tp_operacao_caixa, conta_gerencial_caixa, centro_custo_caixa, deposito_estoque_caixa")
  ...
  const carregarPedidos = useCallback(async () => {
    const { data, error } = await db.from("movimento")
      .select("movimento_id, nr_movimento, cadastro_id, vl_movimento, dt_emissao")
    ...
    let cadMap: Record<number, string> = {};
    if (cadIds.length) {
      const { data: cads } = await db.from("cadastro")
        .select("cadastro_id, razao_social, nome_fantasia").in("cadastro_id", cadIds);
      for (const c of (cads || []) as any[]) cadMap[c.cadastro_id] = c.nome_fantasia || c.razao_social;
    }
  ...
  // ===== Receber: prepara pedido (do grid OU venda direta) e abre pagto =====
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-card">
        ...
        <button onClick={onSair} className="text-sm px-3 py-1 rounded border border-border flex items-center gap-1 hover:bg-accent">
          <LogOut size={14} /> Sair
        </button>
      </div>
      ...
      <div className="flex-1 grid grid-cols-12 gap-3 p-3 overflow-hidden">
        {/* Coluna 1: Pedidos a receber */}
        <div className="col-span-4 flex flex-col border border-border rounded bg-card overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto">
            {XPedidos.length === 0 && (
              <div className="p-4 text-xs text-muted-foreground text-center">Nenhum pedido fechado.</div>
            )}
            {XPedidos.map(p => {
              ...
              return (
                <button key={p.movimento_id}
                  ...
                  className={`w-full text-left px-3 py-2 border-b border-border text-xs ${sel ? "bg-primary/15" : "hover:bg-accent/50"}`}>
        ...
        {/* Coluna 2: Venda direta OU detalhes do pedido */}
        <div className="col-span-5 flex flex-col border border-border rounded bg-card overflow-hidden">
          <div className="px-3 py-2 border-b border-border flex items-center justify-between">
            ...
            {XPedidoSel ? (
              <div className="divide-y divide-border">
                {XPedidoSelItens.map((it: any) => (
                  <div key={it.movimento_item_id} className="px-3 py-2 text-xs">
        ...
        {/* Coluna 3: Resumo */}
        <div className="col-span-3 flex flex-col border border-border rounded bg-card overflow-hidden">
          ...
          <div className="p-3 border-t border-border">
            <button onClick={abrirPagto} disabled={!podeReceber}
      ...
      <PagamentoDialog open={XOpenPagto} totalPedido={totalReceber}
        onClose={() => setXOpenPagto(false)} onConfirmar={confirmarPagamento} />
    </div>
  );
};
